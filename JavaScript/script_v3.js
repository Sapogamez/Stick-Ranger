// Stick Ranger RPG Script v3 - Complete Working Version
...
function updatePlayerCard(card, className) {
    ...
    try {
        ...
        const statSpans = card.querySelectorAll('.stat-value');
        if (statSpans.length < 5) {
            debugLog(`ERROR: Expected 5 stat spans, found ${statSpans.length}`, 'error');
            return;
        }
        ...
        if (className === 'Priest') {
            statSpans[1].textContent = `Heal: ${stats.HEAL}`;
        } else {
            statSpans[1].textContent = stats.ATK + (auraBonus ? ` (+${auraBonus})` : '');
        }
        statSpans[2].textContent = stats.DEF;
        statSpans[3].textContent = stats.SPD;
        statSpans[4].textContent = stats.RANGE;
...
function initializeMap() {
    ...
    if (!minimap || !fullscreenMap || !closeMapBtn) {
        debugLog('ERROR: Map elements not found', 'error');
        return;
    }
...
function spawnEnemies(zoneName) {
    ...
    if (!container) {
        debugLog('ERROR: Enemy container not found', 'error');
        return;
    }
    ...
    zoneEnemies.forEach(enemyType => {
        ...
        for (let i = 0; i < count; i++) {
            ...
            scaledType._startX = 60 + Math.random() * 30; // 60-90% right
            scaledType._startY = 30 + Math.random() * 40; // 30-70% vertical
...
function addHealthBarsToPlayers() {
    ...
    players.forEach((playerElement, index) => {
        if (!playerElement.querySelector('.player-health-bar')) {
            const healthBar = document.createElement('div');
...
let playerHealths = [100, 80, 70, 90]; // Initialize with class defaults
...
function processCombatTurn() {
    ...
    playerElements.forEach((element, index) => {
        ...
        if (!element._initialized) {
            ...
            element._x = 10 + index * 8; // 10, 18, 26, 34% left
            element._y = 40 + index * 3; // 40, 43, 46, 49% vertical
            playerHealths[index] = stats.hp; // Initialize health
        }
    ...
    enemies.forEach(enemy => {
        ...
        // Simple enemy AI - move toward nearest player and attack
        ...
        if (nearestPlayer) {
            const rangeThreshold = enemy.range * 2;
            if (nearestDistance > rangeThreshold) {
                // Move toward player
                const moveSpeed = enemy.spd * 0.5;
                const newPos = moveTowards(enemy, nearestPlayer, moveSpeed);
                enemy.x = Math.max(5, Math.min(95, newPos.x));
                enemy.y = Math.max(15, Math.min(75, newPos.y));
                updateEnemyPosition(enemy);
            } else {
                // Attack player
                const now = Date.now();
                const attackCooldown = 3000 - (enemy.spd * 30);
                if (now - enemy.lastAttack > attackCooldown) {
                    const damage = Math.max(1, enemy.atk - nearestPlayer.stats.def);
                    playerHealths[nearestPlayer.index - 1] = Math.max(0, playerHealths[nearestPlayer.index - 1] - damage);
                    updatePlayerHealth(nearestPlayer.index, playerHealths[nearestPlayer.index - 1], nearestPlayer.maxHp);
                    addCombatLog(`${enemy.name} attacks Player ${nearestPlayer.index} for ${damage} damage!`);
                    
                    if (playerHealths[nearestPlayer.index - 1] <= 0) {
                        addCombatLog(`Player ${nearestPlayer.index} is defeated!`);
    ...
    const alivePlayers = playerHealths.filter(hp => hp > 0).length;
    
    if (aliveEnemies === 0) {
        addCombatLog('Victory! All enemies defeated!');
        // Spawn new enemies or advance level
        gameLevel++;
        enemyStatMultiplier += 0.2;
        setTimeout(() => spawnEnemies(currentZone), 2000);
    } else if (alivePlayers === 0) {
        addCombatLog('Defeat! All players have fallen!');
        toggleAutoBattle(); // Stop auto battle
    }
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    debugLog('Script v3 loaded, DOM ready, initializing systems...', 'info');
    ...
    debugLog('Initializing player cards...', 'info');
    ...
    document.querySelectorAll('.player-card').forEach((card, index) => {
        ...
        // Initial update
        debugLog(`Card ${index + 1}: Performing initial update with class '${currentClass}'`, 'info');
        updatePlayerCard(card, currentClass);
    ...
    debugLog('Game v3 fully initialized and ready!', 'success');
});
